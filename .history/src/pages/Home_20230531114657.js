...
const Home = () => {
  ...
  useEffect(() => {
    ...
    loader.load().then(() => {
      ...
      marker1.addListener("click", async () => {
        ...
        const targetTemperature = fetchedData.avg_temp;
        const targetHumidity = fetchedData.avg_humidity;

        const contentString = `
        <div>
        <h1> <b> Sensor Location: ${location}</b> </h1>
          <h2> Temperature: ${targetTemperature}&deg;C </h2>
          <h2> Humidity: ${targetHumidity}% </h2>
