  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Convert fetched data to the desired format for temperature
        const humidityTableData = Object.entries(fetchData).map(
          ([date, values]) => ({
            date: date,
            Humidity: parseFloat(values.avg_humidity),
          })
        );

        setTemperatureData(temperatureTableData);
        setHumidityData(humidityTableData);
      } catch (error) {
        console.error(error);
      }
    };
